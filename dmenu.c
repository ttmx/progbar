...
drawmenu(void)
{
	...
	drw_rect(drw, mw*(1-len)/2, 2, mw*len, mh-4, 1, 1);

	drw_setscheme(drw, scheme[SchemeSel]);
	drw_rect(drw, mw*(1-len)/2, 2, mw*len*percent/100, mh-4, 1, 1);
...
usage(void)
{
	fputs("usage: dprog [-bfiv] [-m monitor] [-w windowid]\n"
	      "             [-nb color] [-nf color] [-sb color] [-sf color] bar%\n", stderr);
	exit(1);
}
...
main(int argc, char *argv[])
{
	...
	setup();
	struct timespec ts = { .tv_sec = sleeptime, .tv_nsec = 0  };
	nanosleep(&ts,NULL);
	cleanup();
	return 0;
}
